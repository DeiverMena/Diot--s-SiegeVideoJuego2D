...
public class EnemigoVoladorController : MonoBehaviour
{
 ...
 // Start is called before the first frame update
    ...
    void Update()
    {
     ...
     //   if(Vector3.Distance(transform.position, AnimacionPersonaje.instance.transform.position) > distanceToAttack){
                attack = Vector3.zero;
          ...
          /*  }else{
                ...
                if(attack == Vector3.zero){
                    attack = AnimacionPersonaje.instance.transform.position;
                }
                transform.position = Vector3.MoveTowards(transform.position, attack, chaseSpeed * Time.deltaTime);

                if(Vector3.Distance(transform.position, attack) <= 0.1f){
                    hasAttack = true;
                    attackCounter = waitAttack;
                    attack = Vector3.zero;
                }
            }*/
